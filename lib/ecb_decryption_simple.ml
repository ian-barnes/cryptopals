...
module Server = struct
  (* Part 1: the server. The oracle is a function that takes arbitrary input
     from a client, appends the unknown string, PKCS#7 pads the result, encrypts
     it with an unknown key using AES/ECB, and returns the resulting ciphertext. *)

  let blocksize = 16

  let key = Ecb_cbc_detection_oracle.random_aes_key ()

  (* let unknown = File.read_all "12.txt" |> Base64.of_base64 *)
  let unknown =
